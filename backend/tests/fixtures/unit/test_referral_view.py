...
def test_buyer_can_retrieve_own_referral():
    ...
    assert referrals.can_view_or_edit_referral(100, 123, 'digital.gov.au', current_user)


def test_buyer_cannot_retrieve_others_referral():
    current_user = CurrentUser(u'200', 'buyer', None, 'felix@digital.gov.au')
    assert not referrals.can_view_or_edit_referral(100, 123, 'agency.gov.au', current_user)
...
def test_supplier_cannot_retrieve_referral_belonging_to_other_supplier():
    ...
    assert not referrals.can_view_or_edit_referral(100, 123, 'digital.gov.au', current_user)
...
def test_admin_cannot_view_referral_created_by_other_agency():
    current_user = CurrentUser(u'400', 'admin', None, 'alex@digital.gov.au')
    assert not referrals.can_view_or_edit_referral(100, 123, 'other.gov.au', current_user)


def test_buyer_cannot_accept_referral():
    assert not referrals.can_transition_to_state('buyer', 'accepted')


def test_admin_cannot_accept_referral():
    assert not referrals.can_transition_to_state('admin', 'accepted')


def test_supplier_accept_referral():
    assert referrals.can_transition_to_state('supplier', 'accepted')
